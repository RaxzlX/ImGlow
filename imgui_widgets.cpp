...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        {
            const float pad = ImMax(0.0f, IM_TRUNC(square_sz / 10.0f));
            const float rounding = 3.5f; // Adjust the rounding radius as needed

            ImVec2 min = check_bb.Min + ImVec2(pad, pad);
            ImVec2 max = check_bb.Max - ImVec2(pad, pad);
            window->DrawList->AddRectFilled(min, max, check_col, rounding);
...
//added
bool ImGui::SliderScalarGlow(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, bool glow)
{
    ...
    const float circle_radius = 10.0f; // Adjust radius as needed
    ...
    // Define the glow parameters
    int glow_layers = 30; // Number of layers for smooth glow
    float max_glow_thickness = 2.5f; // Maximum thickness of the glow
    ImVec4 glow_color = ImVec4(0.2f, 0.7f, 1.0f, 0.1f); // Glow color with alpha

    // Get the top-left and bottom-right corners of the grab
    ImVec2 grab_min = ImVec2(frame_bb.Min.x, frame_bb.Min.y);
    ImVec2 grab_max = ImVec2(grab_bb.Max.x + 1, grab_bb.Max.y + 1);
    ...
    for (int i = 0; i < glow_layers; ++i)
    {
        ...
        // Expand the rectangle for the current layer
        ImVec2 expanded_min = ImVec2(grab_min.x - thickness, grab_min.y - thickness);
        ImVec2 expanded_max = ImVec2(grab_max.x + thickness, grab_max.y + thickness);

        // Draw the rounded rectangle for this layer
    ...
    // Draw the main grab control on top
    window->DrawList->AddRectFilled(
        grab_min,
        grab_max,
    ...
    //  window->DrawList->AddCircleFilled(
      //    grab_center,
     //     circle_radius,
     //     GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_Text)); // Circle fill color
...
//added
bool ImGui::SliderFloatGlow(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return ImGui::SliderScalarGlow(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, true);
}
